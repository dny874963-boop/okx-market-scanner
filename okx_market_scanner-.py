#!/usr/bin/env python3
"""
OKX Market Scanner - SOL-style swing setup filter

Public market-data scanner only.
Does not trade. Does not use API keys.

WATCH_TRIGGER only if:
1) active TP reward >= $50
2) risk is between $15 and $25
3) active reward/risk >= 1:2
4) trigger is close enough to current price

Setup grading:
- A_SETUP: active reward >= $70 and reward/risk >= 1:3
- B_SETUP: active reward >= $50 and reward/risk >= 1:2
...
MIN_ACTIVE_REWARD_USD = 50.0
QUALITY_REWARD_USD = 70.0

MIN_RISK_USD = 15.0
MAX_RISK_USD = 25.0

MIN_REWARD_RISK = 2.0
QUALITY_REWARD_RISK = 3.0
...
ATR_STOP_MULT = 1.15
MAX_ACTIVE_TP_PCT_FROM_TRIGGER = 18.0
...
class ScanRow:
    ...
    quantity: float | None
    risk_usd: float | None
    active_reward_usd: float | None
    reward2_usd: float | None
    reward_risk: float | None
    quality_reward_risk: float | None
...
def error_row(symbol: str, rank: int | None, price: float | None, volume: float | None, reason: str, note: str) -> ScanRow:
    return ScanRow(
        ...
        quantity=None,
        risk_usd=None,
        active_reward_usd=None,
        reward2_usd=None,
        reward_risk=None,
        quality_reward_risk=None,
...
def scan_symbol(ticker: MarketTicker, universe_rank: int) -> ScanRow:
    ...
    try:
        ...
        active_tp_distance = max(
            risk_per_coin * MIN_REWARD_RISK,
            MIN_ACTIVE_REWARD_USD / qty,
        )
        tp1 = trigger + active_tp_distance
        active_reward_usd = active_tp_distance * qty
        reward_risk = safe_ratio(active_reward_usd, risk_usd)

        quality_tp_distance = max(
            risk_per_coin * QUALITY_REWARD_RISK,
            QUALITY_REWARD_USD / qty,
        )
        tp2 = trigger + quality_tp_distance
        reward2_usd = quality_tp_distance * qty
        quality_reward_risk = safe_ratio(reward2_usd, risk_usd)

        distance_to_trigger_pct = ((trigger - price) / price) * 100.0
        active_tp_pct_from_trigger = ((tp1 - trigger) / trigger) * 100.0

        fail_reasons: list[str] = []
        if risk_usd < MIN_RISK_USD:
            fail_reasons.append("risk_lt_15")
        if risk_usd > MAX_RISK_USD:
            fail_reasons.append("risk_gt_25")
        if active_reward_usd < MIN_ACTIVE_REWARD_USD:
            fail_reasons.append("active_reward_lt_50")
        if reward_risk is None or reward_risk < MIN_REWARD_RISK:
            fail_reasons.append("rr_lt_2_0")
        ...
        if active_tp_pct_from_trigger > MAX_ACTIVE_TP_PCT_FROM_TRIGGER:
            fail_reasons.append("active_tp_too_far_pct")

        filters_passed = not fail_reasons

        setup_grade = "NO"
        if filters_passed:
            if active_reward_usd >= QUALITY_REWARD_USD and reward_risk is not None and reward_risk >= QUALITY_REWARD_RISK:
                setup_grade = "A_SETUP"
            else:
                setup_grade = "B_SETUP"

        note = "SOL-style swing watch" if filters_passed else "Rejected by SOL-style filters"
        ...
        return ScanRow(
            ...
            active_reward_usd=round(active_reward_usd, 2),
            reward2_usd=round(reward2_usd, 2),
            reward_risk=round(reward_risk, 2) if reward_risk is not None else None,
            quality_reward_risk=round(quality_reward_risk, 2) if quality_reward_risk is not None else None,
...
def write_json(rows: list[ScanRow], universe_size: int, path: str = "okx_market_report.json") -> None:
    ...
    near_misses = [
        ...
        and rw.active_reward_usd is not None
        and rw.reward_risk is not None
        and MIN_RISK_USD <= rw.risk_usd <= MAX_RISK_USD
        and rw.active_reward_usd >= MIN_ACTIVE_REWARD_USD
        and rw.reward_risk >= MIN_REWARD_RISK
    ][:25]

    payload = {
        "generated_at_utc": now_utc_iso(),
        "scanner": "OKX Market Scanner - SOL-style swing setup filter",
        "does_trade": False,
        ...
        "rules": {
            "active_tp": "tp1",
            "quality_tp": "tp2",
            "min_active_reward_usd": MIN_ACTIVE_REWARD_USD,
            "quality_reward_usd": QUALITY_REWARD_USD,
            "risk_usd_range": [MIN_RISK_USD, MAX_RISK_USD],
            "min_reward_risk": MIN_REWARD_RISK,
            "quality_reward_risk": QUALITY_REWARD_RISK,
            "max_distance_to_trigger_pct": MAX_DISTANCE_TO_TRIGGER_PCT,
            "max_active_tp_pct_from_trigger": MAX_ACTIVE_TP_PCT_FROM_TRIGGER,
        },
        "summary": {
            ...
            "a_setup_count": sum(1 for x in watch if x.get("setup_grade") == "A_SETUP"),
            "b_setup_count": sum(1 for x in watch if x.get("setup_grade") == "B_SETUP"),
...
def print_summary(rows: list[ScanRow], universe_size: int) -> None:
    ...
    print(f"Rules: active TP1 reward >= ${MIN_ACTIVE_REWARD_USD:.0f} | risk ${MIN_RISK_USD:.0f}-${MAX_RISK_USD:.0f} | active R/R >= {MIN_REWARD_RISK} | trigger distance <= {MAX_DISTANCE_TO_TRIGGER_PCT}%")
    print("-" * 120)

    for row in rows:
        risk = "NA" if row.risk_usd is None else f"${row.risk_usd}"
        active_reward = "NA" if row.active_reward_usd is None else f"${row.active_reward_usd}"
        ...
        tp1 = "NA" if row.tp1 is None else row.tp1
        ...
        print(f"{row.symbol} | grade {row.setup_grade} | rank {row.universe_rank} | vol {volume} | price {price} | status {row.status} | trigger {trigger} | SL {sl} | TP1 {tp1} | TP2 {tp2} | risk {risk} | active_reward {active_reward} | RR {rr} | fail {row.fail_reasons or '-'}")
